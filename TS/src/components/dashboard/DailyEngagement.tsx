  ...
  const chartOptions: ApexCharts.ApexOptions = {
    chart: {
      ...
      dropShadow: chartType !== 'bar'
        ? { enabled: true, color: '#ff6b00', top: 12, blur: 14, opacity: 0.15 }
        : { enabled: false },
    },
    theme: { mode: 'dark' },
    stroke: chartType === 'bar'
      ? { width: 0 }
      : { curve: 'smooth', width: [3, 2] },
    fill: chartType === 'area'
      ? {
          type: 'gradient',
          gradient: {
            ...
            colorStops: [
              [
                ...
                { offset: 100, color: '#22c55e', opacity: 0.01 },
              ],
              [
      ...
      : { type: 'solid', opacity: chartType === 'bar' ? 0.85 : 0 },
    colors: ['#22c55e', '#ef4444'],
    markers: chartType === 'bar'
      ? { size: 0 }
      : {
          size: 5,
          colors: ['#22c55e', '#ef4444'],
    ...
    <div>
      ...
      <div style={{ ...S.card, overflow: 'hidden' }}>
          ...
          <div style={{ display: 'flex', gap: '0.3rem', alignSelf: 'center' }}>
            {(['area', 'line', 'bar'] as const).map((t) => (
              <button
                ...
                style={{
                  background: chartType === t ? '#ff6b00' : '#1a1a1a',
                  border: `1px solid ${chartType === t ? '#ff6b00' : '#2a2a2a'}`,
        ...
        <div style={{ padding: '0.5rem 0.5rem 0' }}>
          <ReactApexChart type={chartType} height={300} series={chartSeries} options={chartOptions} />
        </div>
      </div>
    </div>
  )
