...
interface VideoRecorderProps {
  ...
  onVideoUpload?: (url: string) => void; // ✅ new prop
}
...
const VideoRecorder: React.FC<VideoRecorderProps> = ({
    ...
    async function startStream() {
      try {
        ...
        mediaRecorder.onstop = async () => {
          const fullBlob = new Blob(chunks.current, {
            type: 'video/webm',
          });
  ...
  const uploadFinalVideo = async (blob: Blob) => {
    try {
      ...
      if (result?.videoUrl && onVideoUpload) {
        onVideoUpload(result.videoUrl); // ✅ send video URL to parent
      }
    } catch (error) {
  ...
  if (!hasPermission) {
    return (
      <p style={{ color: 'red' }}>
        ⚠️ Please allow access to your camera and microphone to record the
        interview.
      </p>
    );
  }
