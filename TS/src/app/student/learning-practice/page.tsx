...
const EnglishPractice = () => {
  ...
  // OR a standalone "learningPractice" module purchase both unlock the full
  // 30-attempt monthly allowance for Listening + Reading — server-enforced
  // in learningRoutes.js. Shown here on the hub overview so a student sees
  // the price before drilling into either sub-feature.
  ...
  const buyModule = (plan: ModulePlan) => {
    ...
    fetch(`${baseURL}/api/student/module-access/create-order`, {
      ...
      body: JSON.stringify({ moduleKey: "learningPractice", plan }),
    })
      .then((r) => r.json())
      .then((order) => {
          ...
          handler: async (response: any) => {
            try {
              const verifyRes = await fetch(`${baseURL}/api/student/module-access/verify`, {
                method: "POST",
                headers: { "Content-Type": "application/json", Authorization: `Bearer ${token}` },
                body: JSON.stringify({ ...response, moduleKey: "learningPractice", plan }),
              })
          ...
          <div>
            ...
            <div style={{
              ...
              <div style={{ zIndex: 2, paddingBottom: 28 }}>
                <h1 style={{ fontSize: 36, fontWeight: 900, color: "#0f172a", margin: "0 0 8px", lineHeight: 1.1, display: "flex", alignItems: "center", gap: 10, flexWrap: "wrap" }}>
                  English Practice
                  {!hasAccess && (
                    <span title="Unlock this module, or subscribe to a full plan" style={{ fontSize: 12, fontWeight: 800, background: ORANGE, color: "#fff", borderRadius: 20, padding: "4px 12px", display: "inline-flex", alignItems: "center", gap: 5 }}>
                      🔒 PREMIUM MODULE
                    </span>
                  )}
                </h1>
                ...
                <div style={{ display: "flex", alignItems: "center", gap: 20, flexWrap: "wrap" }}>
                  ...
                  {!hasAccess && (() => {
                    ...
                    return (
                      <div>
                        <div style={{ display: "inline-flex", alignItems: "center", gap: 0, background: "#fff", border: "1px solid #f0d9c0", borderRadius: 10, padding: 4 }}>
