...
import { FiFileText, FiBookOpen, FiMessageCircle, FiUpload, FiCpu, FiList, FiCheckCircle, FiTrendingUp, FiLock, FiArrowRight } from 'react-icons/fi'
...
const ProblemStatement = () => {
  ...
  // OR a standalone "codeChallenge" module purchase both unlock everything
  // past the first FREE_PROBLEMS_LIMIT problems — server-enforced in
  // adminProblems.js (GET /:id) and ai.js (POST /save), not just this flag.
  ...
  // Kept as `isPending` for ProblemsList/test-case-panel props below — the
  // semantics are now "should the free-trial gating apply", not literally
  // account-pending, but both call sites just treat it as a boolean gate.
  ...
  const buyModule = (plan: ModulePlan) => {
    ...
    fetch(`${baseURL}/api/student/module-access/create-order`, {
      ...
      body: JSON.stringify({ moduleKey: 'codeChallenge', plan }),
    })
      .then((r) => r.json())
      .then((order) => {
          ...
          handler: async (response: any) => {
            try {
              const verifyRes = await fetch(`${baseURL}/api/student/module-access/verify`, {
                method: 'POST',
                headers: { 'Content-Type': 'application/json', Authorization: `Bearer ${token}` },
                body: JSON.stringify({ ...response, moduleKey: 'codeChallenge', plan }),
              })
      ...
      <Container fluid className="py-3 px-4">
        ...
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 24, flexWrap: 'wrap', gap: 16 }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 14 }}>
            ...
            <div>
              <h4 style={{ fontWeight: 800, color: 'var(--dash-text, #0f172a)', margin: 0, fontSize: '1.3rem', lineHeight: 1.2, display: 'flex', alignItems: 'center', gap: 8, flexWrap: 'wrap' }}>
                Code Challenge
                {!hasAccess && (
                  <span title="Unlock this module, or subscribe to a full plan" style={{ display: 'inline-flex', alignItems: 'center', gap: 4, color: '#ff7a00', fontSize: '0.85rem', fontWeight: 700 }}>
                    <FiLock size={13} />(Premium Module)
                  </span>
                )}
              </h4>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 12, flexWrap: 'wrap' }}>
            ...
            {/* Plan/price picker — placed right after the stat boxes when locked */}
            {!hasAccess && (() => {
              ...
              return (
                <div style={{ display: 'flex', alignItems: 'center', gap: 0, background: '#fff', border: '1px solid #f0d9c0', borderRadius: 10, padding: 4, flexShrink: 0 }}>
                  ...
                  >
                    {isBusy ? 'Processing…' : <>Buy Now <FiArrowRight size={12} /></>}
                  </button>
                </div>
              )
            })()}
