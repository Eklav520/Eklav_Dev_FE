...
const InterviewDetailsPageView = () => {
  ...
  // OR a standalone "jobBoard" module purchase both unlock live external
  // listings + applying to campus-posted jobs — server-enforced in app.js
  // (GET /api/jobs/external, POST /jobs/:jobId/apply), not just these flags.
  ...
  // Kept as isApproved/isPending below — every existing lock check on this
  // page just treats them as "should the free-trial gating apply", the
  // literal account-status meaning no longer matters.
  const isApproved = hasAccess
  ...
  const buyModule = (plan: ModulePlan) => {
    ...
    fetch(`${baseURL}/api/student/module-access/create-order`, {
      ...
      body: JSON.stringify({ moduleKey: 'jobBoard', plan }),
    })
      .then((r) => r.json())
      .then((order) => {
          ...
          handler: async (response: any) => {
            try {
              const verifyRes = await fetch(`${baseURL}/api/student/module-access/verify`, {
                method: 'POST',
                headers: { 'Content-Type': 'application/json', Authorization: `Bearer ${token}` },
                body: JSON.stringify({ ...response, moduleKey: 'jobBoard', plan }),
              })
  ...
  return (
    <div style={{ minHeight: '100vh', background: PAGE_BG }}>
      ...
      <div style={{ background: CARD_BG, borderBottom: `1px solid ${PAGE_BORDER}`, padding: '14px 24px', display: 'flex', alignItems: 'center', gap: 14 }}>
        <div>
          <div style={{ fontWeight: 800, color: PAGE_TEXT, fontSize: '1.15rem', lineHeight: 1, display: 'flex', alignItems: 'center', gap: 8, flexWrap: 'wrap' as const }}>
            All Jobs
            {!hasAccess && (
              <span title="Unlock this module, or subscribe to a full plan" style={{ display: 'inline-flex', alignItems: 'center', gap: 4, color: '#ff7a00', fontSize: '0.75rem', fontWeight: 700 }}>
