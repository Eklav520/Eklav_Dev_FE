...
type ListeningLeaderboardProps = {
  weekKey: string
  college: string | null
}

const ListeningLeaderboard = ({ weekKey, college }: ListeningLeaderboardProps) => {
  ...
  useEffect(() => {
    const fetchRanking = async () => {
      try {
        ...
        // ✅ append college if present
        if (college) {
          query += `&college=${encodeURIComponent(college)}`
        }

        const res = await fetch(
          `${baseURL}/api/listeningRanking/weekly?${query}`,
          {
            headers: {
