...
export default function AdminCreateProblem({ defaultExamId, readOnly }: { defaultExamId?: string; readOnly?: boolean } = {}) {
  ...
  const downloadTemplate = () => {
    // Row 0: section label headers (merged visually via column span in value)
    const sectionRow = [
      "CHALLENGE INFO", "",
      "TEST CASE 1", "", "", "",
      "TEST CASE 2", "", "", "",
    ...
    const example = [
      "Sum of Two Numbers",
      "Given two numbers a and b as separate lines, print their sum.",
      "1\n2", "3", 10, "trimmed",
      "5\n7", "12", 10, "trimmed",
      "0\n0", "0", 10, "trimmed",
      "abc\ndef", "",
    ...
    const ws = XLSX.utils.aoa_to_sheet([sectionRow, colHeaders, example]);
    ws["!merges"] = [
      { s: { r: 0, c: 0 }, e: { r: 0, c: 1 } },
      { s: { r: 0, c: 2 }, e: { r: 0, c: 5 } },
      { s: { r: 0, c: 6 }, e: { r: 0, c: 9 } },
  ...
  return (
    <div style={{ padding: '1.5rem', maxWidth: 1100, margin: '0 auto' }}>
      ...
      >
        ...
        <Modal.Body style={{ background: "#0d0d0d", color: "#fff", overflowY: "auto", padding: "2rem 3rem" }}>
          <Form>
                ...
                <div key={i} style={{ background: "#111", border: "1px solid #222", borderRadius: 10, padding: "1rem", marginBottom: "0.75rem" }}>
                  ...
                  <Row className="g-2">
                    ...
                    <Col md={8}>
                      ...
                      <Form.Select value={t.matchType} onChange={(e) => updatePositive(i, "matchType", e.target.value as MatchType)}
                        style={{ background: "#1a1a1a", color: "#fff", borderColor: "#2c2c2c", fontSize: "0.85rem" }}>
