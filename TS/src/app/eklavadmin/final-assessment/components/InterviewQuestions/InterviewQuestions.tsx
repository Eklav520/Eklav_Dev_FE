...
type IQProps = {
  /** which API to hit: /api/tr or /api/hr */
  apiBase?: 'tr' | 'hr';
};

const AdminInterviewQuestionsUpload: React.FC = ({ apiBase = 'tr' }: IQProps) => {
  ...
  const handleUpload = async () => {
    ...
    try {
      ...
      const res = await fetch(`${baseURL}/admin/tr/upload-questions`, {
        method: 'POST',
        headers: {
  ...
  const downloadTemplate = () => {
    // CSV template you can open in Excel and "Save As" .xlsx
    const csv = [
      'Topic,Question',
    ...
    a.href = url
    a.download = 'InterviewQuestionsTemplate.csv'
    a.click()
  ...
  return (
    <div className="p-4">
      <h4>📥 Upload Interview Questions</h4>
        ...
        >
          ...
          {(result.processed !== undefined ||
            ...
            <div className="mt-2 small">
              {result.processed !== undefined && <div><strong>Processed:</strong> {result.processed}</div>}
              {result.inserted !== undefined && <div><strong>Inserted:</strong> {result.inserted}</div>}
              {result.updated !== undefined && <div><strong>Updated:</strong> {result.updated}</div>}
              {result.skipped !== undefined && <div><strong>Skipped:</strong> {result.skipped}</div>}
            </div>
          )}
        </Alert>
      )}
