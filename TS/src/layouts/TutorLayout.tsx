...
import { TUTOR_MENU_ITEMS } from '@/assets/data/menu-items'
...
const TutorLayout = ({ children }: ChildrenType) => {
  ...
  useEffect(() => {
    ...
    fetch(`${baseURL}/profile`, {
      ...
      .then((profile) => {
        setName(profile.fullName || 'Tutor')
        setRole(profile.role || 'Tutor')
...
const VerticalMenu = ({ onItemClick }: { onItemClick?: () => void }) => {
  ...
  const baseMenu: MenuItemTypeLocal[] = useMemo(() => {
    return TUTOR_MENU_ITEMS as unknown as MenuItemTypeLocal[]
  }, [])
